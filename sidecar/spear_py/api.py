"""
SAPPS Sidecar - FastAPI service for ML-based similarity detection
...
SYSTEM_PROMPT = os.getenv("SAPPS_SYSTEM_TEXT", "")
if SYSTEM_PROMPT:
    system_embedding = model.encode([SYSTEM_PROMPT], normalize_embeddings=True)
else:
    system_embedding = None
    logger.warning("SAPPS_SYSTEM_TEXT not set - similarity checks will use request reference")

app = FastAPI(title="SAPPS Sidecar", version="0.1.0")
...
async def check_similarity(request: SimilarityRequest):
    ...
    try:
        ...
        if not reference:
            raise HTTPException(
                status_code=400,
                detail="No reference text provided and SAPPS_SYSTEM_TEXT not set"
            )
